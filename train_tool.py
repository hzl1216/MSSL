...
def get_permutation_importance(model,dataset):
    ...
    for index,feature_name in enumerate(feature_names[::split]):
        ...
        test_loader = data.DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
        test_acc = test2(test_loader, model)


        dataset.data[:, start:end] = origin
        importances.append((feature_name, benck[1]-test_acc[1], benck[0] ))
...
def test(val_loader, model):
    ...
    _, pred = all_outputs.topk(1, 1, True, True)
    precison = precision_score(pred.cpu().numpy(), all_labels.cpu().numpy(), average='weighted')
    f1 = f1_score(pred.cpu().numpy(), all_labels.cpu().numpy(), average='weighted')
    recall = recall_score(pred.cpu().numpy(), all_labels.cpu().numpy(), average='weighted')
...
def test2(val_loader, model):
    ...
    return all_labels.cpu().numpy(),all_outputs.cpu().numpy()
